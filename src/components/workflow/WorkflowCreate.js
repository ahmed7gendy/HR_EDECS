...
const WorkflowCreate = () => {
  ...
  const loadCompanies = async () => {
    try {
      const companiesData = await CompanyManager.getCompanies();
  ...
  const handleCompanyChange = async (event) => {
    ...
    try {
      const [languagesData, currenciesData] = await Promise.all([
        CompanyManager.getCompanyLanguages(companyId),
        CompanyManager.getCompanyCurrencies(companyId)
      ]);
